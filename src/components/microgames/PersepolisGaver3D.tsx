...
//
// Lyspære: Perserriket var det første verdensriket. Det holdt dusinvis av ulike
// folkeslag sammen ved å la hvert folk beholde sin egen drakt og sine egne skikker,
// mens ALLE bøyde seg for EN konge. Relieffene på Apadana-trappa i Persepolis viser
...
// fram til tronen. Hver har ulik drakt og ulik gave fra sitt land. Når alle fem har
// bøyd seg, er riket "samlet" - ett rike, mange folk, en konge.

type GiftKind = 'vessel' | 'amphora' | 'basket' | 'cloth' | 'tusk';

interface Delegation {
    id: string;
    folk: string; // navnet på folket
...
const DELEGATIONS: Delegation[] = [
    {
        ...
        folk: 'Lyderne',
        gaveKort: 'gullkar',
        fakta: 'Lyderne kom fra Lydia, langt vest i dagens Tyrkia. De var kjent som rike handelsfolk, og ga storkongen fine kar og armbånd av gull.',
        robe: '#6d3b8f',
        skin: '#d9a878',
        gift: 'vessel',
        giftColor: '#f3c64b',
    ...
    },
    {
        ...
        folk: 'Inderne',
        gaveKort: 'gullstøv',
        fakta: 'Fra Indusdalen heilt øst i riket kom inderne med kurver fulle av gullstøv. India var rikets rikeste hjørne på gull.',
        robe: '#d68a2e',
        skin: '#a9713f',
        gift: 'basket',
        giftColor: '#ffd34d',
    },
    {
        id: 'babylonerne',
        folk: 'Babylonerne',
        gaveKort: 'fint vevd tøy',
        fakta: 'Babylon lå ved elvene i dagens Irak og var en av verdens eldste storbyer. Babylonerne ga kongen fint vevd tøy og stoff.',
        robe: '#9c2b3a',
        skin: '#d9a878',
        gift: 'cloth',
        giftColor: '#3f7f7a',
    },
    {
        ...
        folk: 'Nubierne',
        gaveKort: 'elfenbein',
        fakta: 'Nubierne kom heilt fra sør, ved Nilen sør for Egypt. De baar en støtann av elfenbein, en sjelden skatt fra Afrika.',
        robe: '#4a3626',
        skin: '#6b4326',
        gift: 'tusk',
        giftColor: '#efe8d6',
...
// Ruten: eleven drar fra basen (nær kamera) fram til tronen (bak).
const BASE_Z = 5.5;
const PLACED_Z = -3.0;
const THRONE_Z = -6.2;
...
const START_BANNER =
    'Utsendinger fra hele riket venter. Dra det første folket fram til storkongen og la dem bøye seg.';
                ...
                <>
                    ...
                    {!done && (
                        <DataReadout
                            corner="bl"
                            items={[
                                { label: 'Folk som har bøyd seg', value: `${delivered} / 5` },
                                {
                                    label: 'Riket',
                    ...
                    <DragHint show={delivered === 0} corner="bc">
                        Dra den nærmeste utsendingen fram til kongen på tronen
                    </DragHint>
                </>
            }
            scene={
                ...
                {!done ? (
                    <>
                        <p className="text-sm text-slate-600 leading-snug">
                            I Persepolis møttes utsendinger fra hele det store riket for å gi
                    ...
                    <WinScreen title="Det første verdensriket: ett rike, mange folk" onReplay={reset}>
                        Alle fem folkene bøyde seg for samme konge, men fikk beholde sin egen drakt,
                        sitt eget språk og sine egne skikker. Det var nettopp slik Perserriket klarte
                        å holde så mange ulike folk samlet i ett rike. Toleranse og respekt for de
                        erobrede var like viktig som hæren. Derfor kaller vi det verdens første
...
function PersepolisScene({
    ...
    return (
        <group>
            ...
            {/* Persepolis-søyler som flankerer prosesjonen */}
            {[-6.5, 6.5].map((x) =>
            ...
            {/* Storkongen på tronen */}
            <GreatKing delivered={delivered} />

            {/* Allerede leverte delegasjoner: står og bøyer seg for kongen */}
            ...
            {/* Feiring når et folk har levert */}
            <Burst position={[0, 1.6, PLACED_Z]} trigger={burst} color="#ffd34d" count={24} spread={2.6} />

            {/* Lett støv i den varme palassluften */}
            <Particles preset="dust" />
        </group>
    );
}

// Storkongen: sitter på en opphøyd trone under en baldakin, med krone. Dais-en
// gløder sterkere for hvert folk som har bøyd seg (riket vokser).
...
// Persepolis-søyle: høy, slank skaft med et enkelt dyre-kapitel på toppen.
function PersianColumn({ position }: { position: [number, number, number] }) {
    return (
        <group position={position}>
            <Column height={4.6} radius={0.3} color="#e0cf9f" />
            {/* Kapitel-antydning: to liggende blokker på toppen */}
...
// Gaven som utsendingen bærer, holdt foran kroppen i armhøyde.
...
// Litt mørkere variant av en farge (til bein/underdrakt), uten å lese ref-er.
