...
- **Kildeliste (obligatorisk, aller siste blokk — etter Quiz):** Minst 3 kilder. Hver kilde MÅ faktisk hentes og leses mot den virkelige siden — ALDRI konstrueres eller gjettes, heller ikke plausible SNL-forfatternavn eller datoer. Format følger husets APA-stil fra `public/content/norsk/skrivehjelp/hvordan-fore-kilder.json`: kun årstall i parentes (aldri SNL sin "sist oppdatert"-dato), pluss `Hentet DD.MM.ÅÅÅÅ fra URL` for kilder som endres ofte (som SNL). Stabile bok-/artikkelkilder trenger ikke hentedato. **Mangler nettsiden årstall, er det ikke grunn til å droppe kilden:** skriv `(u.å.)` i både henvisning og listeoppføring, og ta alltid med hentedato — det er hentedatoen som gjør en udatert kilde etterprøvbar. Mangler den forfatter, bruk organisasjonen (aldri "ukjent forfatter" eller domenenavnet). Kravet er etterprøvbarhet, ikke dato: dropp en kilde bare når siden ikke lar seg hente, ikke er troverdig, eller ikke dekker påstanden. Eksempel: `Sigurdsson, J. V. (2021). *Vikingtiden*. Store norske leksikon. Hentet 10.01.2024 fra https://snl.no/vikingtiden`.
