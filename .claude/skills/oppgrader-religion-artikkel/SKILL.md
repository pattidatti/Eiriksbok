...
for root, dirs, files in os.walk(base):
    for f in files:
        ...
        has_micro = 'MicroGame' in names
        has_oppg = 'Oppgaver' in names
        has_kilde = 'Kildeliste' in names
        words = count_words(content)

        # FERDIG = alle fire kravene oppfylt OG nok tekst.
        if has_sig and has_micro and has_oppg and has_kilde and words >= 800:
            continue

        score = ((3 if not has_sig else 0)
                 + (2 if not has_micro else 0)
        ...
        results.append(dict(path=path, religion=religion, title=d.get('title', d.get('id', f)),
                            ...
                            has_sig=has_sig, has_micro=has_micro, has_oppg=has_oppg,
...
for r in ordered[:20]:
    if r['broken']:
        status = 'BROKEN'
    elif not r['has_sig'] and not r['has_micro']:
        status = 'EMPTY'
    else:
        status = 'PARTIAL'
    mangler = ','.join(k for k, v in (('sig', r['has_sig']), ('mikro', r['has_micro']),
...
| `EMPTY` | Verken signaturkomponent eller mikrospill |
| `PARTIAL` | Har noe, men mangler minst ett krav |
...
description: "0 ord · ødelagt skjema (renderer tomt) · mangler sig, mikro, oppg, kilde"

label: "Islam: Sentrale trekk i Islam"
description: "100 ord · 6 blokker · mangler sig, mikro, oppg, kilde"
```
...
4. Spill gjennom mikrospillet til **både** `WinScreen` og `LoseScreen`
5. `node scripts/audit-microgames.mjs --ids <gameId> --strict --frames 4` — se på alle fire
   rammene med egne øyne, ikke bare exit-koden
6. Kjør `npm run scan:content` og rydd opp i eventuell churn
7. Verifiser at `public/data/comparison-manifest.json` fortsatt har artikkelen på riktig tema
